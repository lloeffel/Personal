...
usage() {
    ...
    echo '  --verbose                   Display test reports'
    echo '  --no-color                  Disable colored output'
    echo '  --skip-leak-check           Disable valgrind leak check'
    echo '  --no-exit-on-fail           Disable valgrind leak check'
    echo '  --print [input|output|all]  Print test info'
}
...
IS_NO_COLOR=$(parse_flag "${FLAGS[*]}" '--no-color') || exit 1
...
DIFF_COLOR_ARG=$("$IS_NO_COLOR" || echo -n '--color')
...
for TEST_NAME in "${TESTS[@]}"; do
    ...
    TMP_LEAK_REPORT_PATH="$TEST_DIR/leak-report.tmp.txt"
